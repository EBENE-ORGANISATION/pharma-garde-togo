...
export function SignalerDialog({ pharmacie }: { pharmacie: SignalablePharmacie }) {
  const { t } = useLang();
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) reset(); }}>
      <DialogTrigger asChild>
        <button className="inline-flex items-center justify-center gap-1.5 text-xs font-medium text-muted-foreground underline-offset-2 hover:underline">
          <AlertCircle className="h-3.5 w-3.5" /> {t("report_problem")}
        </button>
      </DialogTrigger>
